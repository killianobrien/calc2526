...
\newif\ifshowanswers
\showanswerstrue
% \showanswersfalse %comment this line to show solutions
...
\sheettitle{Tutorial sheet: Sequences \hfill Dr Killian O'Brien}

\vvskip
{\small The computer icon \includegraphics[scale=0.08]{comp.png} indicates where computer calculation or programming may be of use.}
...
a(n) = 3*n/(n^2+1);
\end{sagesilent}
    \solution{The first five terms of the sequence are
...
a(n) = 2^n/(2+2^n);
\end{sagesilent}
    \solution{The first five terms of the sequence area
...
a(n) = 3*n/(n^2+1);
\end{sagesilent}
    \solution{The element $a_{2m+3}$ is given by
...
a(n) = 2^n/(2+2^n);
\end{sagesilent}
    \solution{The element $b_{2m+3}$ is given by
...
a(n) = (-1)^n/(3*n);
\end{sagesilent}
    \solution{The element $c_{2m+3}$ is given by
...
a(n) = (-1)^(n-1)/(2*n);
\end{sagesilent}
    \solution{The element $d_{2m+3}$ is given by
...
a(n) = (-1)^(n-1)/(2*n);
\end{sagesilent}
    \solution{The element $e_{2m+3}$ could be expressed as
...
\solution{It is relatively straightforward to see that a nice solution in terms of factorials for the sequence $\{f_n\}_{n=1}^\infty$ is given by
$$f_n = \frac{3}{(n-1)!},$$
so the appropriate expression for $f_{2m+3}$ is
$$f_{2m+3} = \frac{3}{(2m+2)!}.$$}
\solution{The element $g_{2m+3}$ can be expressed as
$$g_{2m+3} = \frac{g_{2m+2}}{1+g_{2m+2}}.$$}
\solution{The element $h_{2m+3}$ can be written as
$$h_{2m+3} = \frac{3^{2m+3}}{(4m+6)!}.$$}
...
a(n) = 3*n/(n^2+1);
\end{sagesilent}
    \solution{The plot of the beginning of $\left \{ a_n \right \}$ is

    \sageplot[width=.5\textwidth]{list_plot([a(i) for i in (0..50)])}}
\begin{sagesilent}
var('n,m');
a(n) = 2^n/(2+2^n);
\end{sagesilent}
   \solution{The plot of the beginning of $\left \{ b_n \right \}$ is

    \sageplot[width=.5\textwidth]{list_plot([a(i) for i in (1..50)])}}
\begin{sagesilent}
var('n,m');
a(n) = (-1)^n/(3*n);
\end{sagesilent}
     \solution{The plot of the beginning of $\left \{ c_n \right \}$ is
...
a(n) = (-1)^(n-1)/(2*n);
\end{sagesilent}
   \solution{The plot of the beginning of $\left \{ d_n \right \}$ is
...
def a(n):
    if n==0 or n==1:
        ...
        return a(n-1) + a(n-2);
\end{sagesilent}
    \solution{This $\left \{ e_n \right \}$ (the Fibonacci sequence) begins positive and after the second term each term comes from adding a positive quantity to the previous term -- so it is clearly increasing after the second term.
The plot of $\left \{ e_n \right \}$ looks like

\sageplot[width=.5\textwidth]{list_plot([a(i) for i in (0..10)])}}
\begin{sagesilent}
var('n');
def a(n):
    if n==1:
        ...
        return a(n-1)/(n-1);
\end{sagesilent}
\solution{The factorial form of $f_n$ clearly shows that the sequence will be decreasing after the first couple of terms. The plot looks like

\sageplot[width=.5\textwidth]{list_plot([a(i) for i in (1..15)])}}
\begin{sagesilent}
var('n');
def a(n):
    if n==0:
        ...
        return a(n-1)/(1+a(n-1));
\end{sagesilent}
\solution{The plot of the beginning of $\left \{ g_n \right \}$ looks like

\sageplot[width=.5\textwidth]{list_plot([a(i) for i in (1..10)])}}
\begin{sagesilent}
var('n');
def a(n):
    return 3^n/factorial(2*n)
\end{sagesilent}
\solution{The plot of the beginning of $\left \{ h_n \right \}$ looks like

\sageplot[width=.5\textwidth]{list_plot([a(i) for i in (1..8)])}}
    \item
    For those that are increasing or decreasing, try and construct a clear argument (proof) for your conclusion using the ratio and/or difference of consecutive terms.
\solution{The sequences $a$, $b$, $|c|$, $|d|$, $h$ can all be shown to be decreasing/increasing as appropriate by examining the ration of consecutive terms and showing it is always less than or greater than 1 as appropriate. An equivalent approach to could be taken by considering the difference of consecutive terms and comparing to $0$. Let's look at the sequence $a$ as an example:
\begin{align*}
...
For the sequences $e$, $f$ and $g$ we can giver more direct arguments based on the initial term(s) and the nature of the recurrence relation.}
    \end{enumerate}
...
How well do you understand the definition of convergence for a sequence?
    ...
    \item
    Don't be too attached to the symbols $x_n, N, \epsilon$ used in the definition in the notes. Can you reliably quote the definition using entirely different symbols?
    \end{enumerate}
    \mbox{}
\solution{These are discussion questions for the students and class. The type of diagrams I have in mind though  for part (b) are like the following
\includegraphics[scale=0.1]{seqconvg01.jpg}
or
\includegraphics[scale=0.1]{seqconvg02.jpg}.}
\item
...
\solution{In the sequence of terms $1/n^2$ the denominator is clearly increasing and unbounded as $n \to \infty$. Therefore the term will be decreasing and convergent with limit $0$. To prove this formally by the definition of convergence we proceed as follows.
...
\item
Consider the following statements about a sequence $\left \{ y_n \right \}_{n=1}^\infty$ which at first glance appear similar to the definition of sequence convergence.
    \begin{itemize}
    \item[\textbf{S1:}]
    There exists a real value $\lambda > 0$ and a positive integer $M$ such that for all $m > M$, $\left | y_m - L \right | < \lambda$.
\solution{This statment says that for a quantity $L$ there is a fixed distance $\lambda$ and a point $y_M$ in the sequence after which the term $y_m$ is always within a distance $\lambda$ of $L$.
...
\solution{This statement says that after some point $y_M$, the sequence terms are within the distance $\lambda$ of this quantity $L$, and that this is true for \textbf{any} real $\lambda > 0$. How can sequence terms be arbitrarily close to some quanitity $L$? This is only possible if all these sequence elements are actually equal to $L$.

So the statement S2 is true, only of the sequences which are \emph{eventually constant}, i.e. after some point all the sequence elements are equal to this quantity $L$.}
    \end{itemize}
For each of these statements can you give some examples of sequences (and values $L$) that satisfy them? For each statement can you characterize all the sequences that satisfy them?
...
\solution{Parts (a) - (e) can be solved by applying the algebra of limits theorem. The proofs are summarised below but should be supplemented with some commentary as well.}
    \begin{enumerate} \setstretch{1.5}
    \item
    $\left \{ a_n \right \}_{n=0}^\infty$, where $a_n = \frac{3n}{n^2 + 1}$.
    \solution{
...
$$ \sqrt{2}, \,\, \sqrt{2\sqrt{2}}, \,\, \sqrt{2\sqrt{2\sqrt{2}}}, \,\,  \sqrt{2\sqrt{2\sqrt{2\sqrt{2}}}}, \dots .$$
    ...
    \item
    Determine the limit.
    \solution{Since we now know the limit exists let us denote it by $a$. Then by the algebra of limits theorem the limit $a$ satisfies
\begin{align*}
& a = \sqrt{2 a} \\
\Leftrightarrow \, \, & a = 2.
\end{align*}}
...
\solution{Firstly, all sequence elements are bounded below by 1, as $a_0 = 1$ and thereafter $a_n = 3 - 1/a_{n-1} \geq 3 - 1 = 2$. Hence all the elements $a_n$ are positive and so sequence is bounded above by 3.
...
\solution{This results says that every subsequence of a convergent sequence is also convergent and tending to the same limit as the sequence itself. It's an example of something that kind of takes more work to state the result carefully than it does to prove it.
...
\setcounter{enumi}{\value{pass}}
\item
In this question we can apply some of the sequence convergence ideas to approximate $\pi$ using a precise geometric method based on the technique used by Archimedes to estimate $\pi$.
...
\solution{The recurrence relation for $a_n$ is
...
%sagemathcloud={"latex_command":"latexmk -pdf -synctex=1  'tutorial-sheet-01-sequences.tex'"}
